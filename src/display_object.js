...
LightSaber.DisplayObject.prototype = {
    ...
    initVars: function (game, spec, parent) {
        ...
        this.parent = parent;
        this.data = spec;
        this.state = new LightSaber.utils.mapping({x:0, y:0, width: 0, height: 0, pending: {width:!0,height:!0,x:!0,y:!0}});
        this.phaser = {};
    },
    doUpdate: function (delta) {
        this.update(delta);
        for (var i in this.children_ordered) {
            this.children_ordered[i].doUpdate(delta);
        }
    },
    ...
    sortChildren: function () {
        ...
        while (list.length > 0) {
            ...
            for (var i=0; i<deps.length; i++) {
                ...
                var ref = child.resolveReference(deps[i]);
                var dep = ref.getName();
                if (!(dep in ready)) {
        ...
        this.children_ordered = new_order;
    },
    ...
    createGraphics: function () {
        ...
        this.subscribeToEvents();      
        // iteration must be in the order the children where declared.        
        ...
        for (var prop in this.data) {
            if (prop in this.game.saber.tweenable_properties) {
                if (this.data[prop] != this.state[prop]) {
                    this.state.set(prop, this.data[prop]);
                    console.debug("********************************>", prop, this.data[prop]);
                }
            }
        }
        ...
        switch (parts[0]) {
            ...
            default:
                ...
                if (parts[0].indexOf("%") != -1) {
                    oy = parseFloat(parts[0].substr(0, parts[0].indexOf("%"))) * 0.01;
                } else {
                    oy = parseFloat(parts[0]) / this.state.height;
                }
        }
        ...
        switch (parts[1]) {
            ...
            default:
                ...
                if (parts[1].indexOf("%") != -1) {
                    ...
                    ox = parseFloat(parts[1]) / this.state.width;
                }
        }
    ...
    computeDeployment: function (apply) {    
        var result = {
                ...
                x: this.state.x,
                y: this.state.y,
                pending: {width:!0,height:!0,x:!0,y:!0}
            },
            before = {
                width: this.state.width,
                height: this.state.height
        ...
        if (this.data.anchors) {
            ...
            for (var i=0;i<2;i++) {
                ...
                split_my[i] = this.data.anchors[i].my.split(" ");
            }
            ...
            delete result.pending.x;
            delete result.pending.y;
            if (split_my[0][0] != split_my[1][0]) {
                delete result.pending.height;
            }            
            if (split_my[0][1] != split_my[1][1]) {
                delete result.pending.width;
            }            
        }        
    ...
    resize: function () {
        ...
        for (var i in this.children_ordered) {
            this.children_ordered[i].resize();
        }
    },
    update: function () {
